...
{
	Fixed r;
	long tmp = static_cast<long>(_raw) * other._raw;
	r.setRawBits(static_cast<int>(tmp >> _fractBits));
	return (r);
}
Fixed Fixed::operator/(Fixed const &other) const
{
	Fixed r;
	long tmp = (static_cast<long>(_raw) << _fractBits) / other._raw;
